...
from airlinescraper.items import AirlinescraperItem
import re
...
class ScraperSpider(Spider):
    ...
    start_urls = ['https://www.airlinequality.com/airline-reviews/united-airlines/page/' + str(i*3)  + "/" for i in range(100)]
    ...
    def parse(self, response):
        ...
        for review in reviews:
            ...
            #FullRating
            try:
                FullReview = review.xpath('.//div[@class="text_content "]/text()').extract_first()
                Verified = 0
                if(len(FullReview) == 2):
                    FullReview = review.xpath('.//div[@class="text_content "]/text()[2]').extract_first()
                    Verified = 1
            except:
                FullReview = "None"


            item = AirlinescraperItem()
            item['AirName'] = 'Jet Blue'
